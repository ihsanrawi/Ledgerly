...
- [ ] Frontend: Create manual column mapping component (AC: 1, 2, 3)
  - [ ] Create `manual-mapping.component.ts` in `features/import/` module
  - [ ] Implement drag-drop using Angular CDK Drag and Drop
  - [ ] Display CSV headers as draggable pills/chips with Material Design
  - [ ] Create drop zones for required fields (Date, Amount, Description) and optional (Memo, Balance, Account)
  - [ ] Show preview table with first 5 sample rows
  - [ ] Highlight mapped columns in preview table with color-coded badges
  - [ ] Update column highlighting dynamically as user drags headers

- [ ] Frontend: Add mapping validation and save functionality (AC: 4, 5)
  - [ ] Validate required mappings (Date, Amount, Description) before allowing save
  - [ ] Display validation errors if required fields not mapped (red border, error message)
  - [ ] Add "Save Mapping" button with bank identifier input field
  - [ ] Disable "Next" button until required mappings complete
  - [ ] Call `POST /api/import/save-mapping` with bank identifier and mappings
  - [ ] Show success toast: "Mapping saved for [Bank Name]"
  - [ ] Store mapping result in component state for next step

- [ ] Frontend: Create Settings → Import Rules page (AC: 6)
  - [ ] Create `import-rules.component.ts` in `features/settings/` module
  - [ ] Display saved mappings table: Bank Name, Date Created, Mapped Columns, Actions
  - [ ] Add "Edit" button to modify existing mapping
  - [ ] Add "Delete" button to remove mapping with confirmation dialog
  - [ ] Call `GET /api/import/mappings` on component init
  - [ ] Add "Test Mapping" feature to upload CSV and preview with saved rule

- [ ] Frontend: Integrate manual mapping into CSV import workflow (AC: 1, 2, 3, 4)
  - [ ] Update `import-csv.component.ts` to detect low-confidence mappings
  - [ ] If `RequiresManualMapping === true`, route to manual mapping step
  - [ ] Show "Manual Mapping Required" banner with explanation
  - [ ] Pre-fill manual mapping UI with auto-detected mappings (if any)
  - [ ] Allow user to override auto-detected mappings
  - [ ] Route to next step (duplicate detection/category suggestions) after mapping complete

- [ ] Frontend: Write component unit tests (AC: 1-6)
  - [ ] Create `manual-mapping.component.spec.ts`
  - [ ] Test drag-drop functionality (headers to drop zones)
  - [ ] Test preview table column highlighting updates on drag
  - [ ] Test validation errors for missing required mappings
  - [ ] Test "Save Mapping" button disabled/enabled states
  - [ ] Test bank identifier input and save functionality
  - [ ] Create `import-rules.component.spec.ts` for settings page tests
