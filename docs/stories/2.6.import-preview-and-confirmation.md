...
- [ ] Pre-Implementation: Verify required components exist (BLOCKING)
  - [ ] Verify `Money` value object exists in `src/Ledgerly.Api/Common/ValueObjects/Money.cs` (per coding-standards.md Rule #2)
  - [ ] If Money class missing, create it with: `public long Cents { get; private set; }`, constructor from decimal, `ToDecimal()` method
  - [ ] Verify `HledgerFileWriter.BulkAppend()` method exists in `src/Ledgerly.Api/Common/Hledger/HledgerFileWriter.cs`
  - [ ] Verify method signature: `public async Task<WriteResult> BulkAppend(List<Transaction> transactions, string filePath)`
  - [ ] Verify Story 2.5 components: `DetectDuplicatesHandler.cs`, `GetCategorySuggestionsHandler.cs`, `ImportRule` entity, `DuplicateWarningDialogComponent.ts`
  - [ ] If any critical component missing, HALT and notify user before proceeding

- [ ] Backend: Create ConfirmImport command and handler (AC: 4, 5, 6)
  - [ ] Create `ConfirmImportCommand.cs` with validated transaction list
  - [ ] Create `ConfirmImportHandler.cs` to persist transactions to SQLite cache
  - [ ] Create `ConfirmImportEndpoint.cs` with Wolverine HTTP pattern `POST /api/import/confirm`
  - [ ] **CRITICAL:** Add `[Authorize]` attribute to endpoint - requires authenticated user
  - [ ] Filter out transactions marked as duplicates (from Story 2.5 duplicate decisions)
  - [ ] Validate all required fields present: Date, Payee, Amount, Category
  - [ ] **CRITICAL:** Convert Amount (decimal) to Money value object (INTEGER cents) before persistence per coding standards
  - [ ] Insert transactions into `Transactions` table via EF Core with `SyncStatus=PendingWrite`
  - [ ] Wrap SQLite insert and hledger write in EF Core TransactionScope for atomicity
  - [ ] Create `CsvImport` audit record with import statistics (TotalRows, SuccessfulImports, DuplicatesSkipped, ErrorCount)
  - [ ] Trigger async write to .hledger file after cache persistence (see subtask below)
  - [ ] On rollback: Mark transactions as `SyncStatus=WriteError`, rollback entire TransactionScope
  - [ ] Return success response with import summary and transaction IDs

- [ ] Backend: Write imported transactions to .hledger file (AC: 4)
  - [ ] Resolve .hledger file path from UserSettings.HledgerFilePath (default: ~/.ledgerly/ledger.hledger)
  - [ ] Validate .hledger file exists before write operations, throw FileNotFoundException if missing
  - [ ] Use `HledgerFileWriter.BulkAppend()` for performance (batch write optimization)
  - [ ] Generate unique `HledgerTransactionCode` (Guid) for each transaction
  - [ ] Format transactions to hledger syntax: date, transaction code, payee, postings
  - [ ] **Convert Money value object back to decimal for hledger file formatting**
  - [ ] Execute atomic write: write to `.hledger.tmp` → validate with `hledger check` → rename to `.hledger`
  - [ ] Create `.hledger.bak` backup before write
  - [ ] Update transactions in cache with `SyncStatus=InSync` after successful write
  - [ ] Insert `HledgerFileAudit` record (Operation=CsvImport, FileHashBefore, FileHashAfter, TransactionCount)
  - [ ] If validation fails, throw HledgerValidationException to trigger TransactionScope rollback

- [ ] Backend: Write unit tests for ConfirmImport (AC: 4, 5, 6)
  - [ ] Create `ConfirmImportHandlerTests.cs`
  - [ ] Test successful import: 10 transactions → all saved to SQLite with correct data
  - [ ] Test Money value object conversion: $45.23 → 4523 cents in database
  - [ ] Test duplicate filtering: 5 duplicates skipped, 5 new transactions imported
  - [ ] Test validation failure: missing category → returns validation error
  - [ ] Test hledger write success: transactions marked `SyncStatus=InSync`
  - [ ] Test hledger write failure: transactions marked `SyncStatus=WriteError`, entire transaction rolled back
  - [ ] Test CsvImport audit record creation: correct counts (TotalRows, SuccessfulImports, DuplicatesSkipped)
  - [ ] Test HledgerFileAudit record creation: correct operation type and file hashes
  - [ ] Test TransactionScope rollback: SQLite inserts rolled back on hledger validation failure
